import {Data} from "./serialization/data";
import {assertNotNil} from "@telegram/foundation";

interface RsaKey {
    modulus: string;
    exponent: string;
}

export class RsaKeyService {
    private static readonly keys: RsaKey[] = [
        {
            modulus: "",
            exponent: "010001"
        },
        {
            modulus: "",
            exponent: "010001"
        },
        {
            modulus: "",
            exponent: "010001"
        },
        {
            modulus: "",
            exponent: "010001"
        },
        {
            modulus: "",
            exponent: "010001"
        }
    ];
    private loadedKeys: Map<string, RsaKey> | null = null;

    public async loadKeyByFingerprint(fingerprint: bigint): Promise<RsaKey> {
        await this.loadRsaKeysIfNeeded();
        const key = this.loadedKeys!.get(fingerprint.toString(16))!;
        assertNotNil(key);
        return key;
    }

    public async loadRsaKeysIfNeeded() {
        if (!this.loadedKeys) {
            this.loadedKeys = new Map();
        }

        for (const key of RsaKeyService.keys) {
            const bytes = this.readKeyBytes(key);
            const fingerprintBytes = await this.hashBytes(bytes);
            const fingerprint = Data.stringFromBytes(fingerprintBytes);
            this.loadedKeys.set(fingerprint, key);
        }
    }

    private async hashBytes(bytes: Uint8Array): Promise<Uint8Array> {
        const buffer = (await crypto.subtle.digest("SHA-1", bytes)).slice(-8);
        const hashBytes = new Uint8Array(buffer);
        hashBytes.reverse();
        return hashBytes;
    }

    private readKeyBytes(key: RsaKey): Uint8Array {
        const data = Data.allocate({initialCapacity: 256});
        data.writeByteString(key.modulus);
        data.writeByteString(key.exponent);

        return data.select(0, data.offset).bytes;
    }
}
