...
export class RsaKey {
    ...
    constructor({modulus, exponent}: { modulus: string, exponent: string }) {
    ...
    public get bytes(): Uint8Array {
        ...
        data.writeByteString(this.modulus);
        data.writeByteString(this.exponent);
    ...
    private async loadRsaKeysIfNeeded() {
        if (!this.loadedKeys) {
            ...
            for (const key of RsaKeyService.keys) {
                const fingerprintBytes = await this.hashBytes(key.bytes);
